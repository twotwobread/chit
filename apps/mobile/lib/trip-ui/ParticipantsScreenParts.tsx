  ...
  return (
    <Card>
      <View style={styles.sectionHeader}>
        ...
        <Text style={styles.message}>모임 멤버와 이 일정 참여자는 다를 수 있어요.</Text>
      </View>
      {removeErrorMessage ? <Text style={styles.errorMessage}>{removeErrorMessage}</Text> : null}
      {removingParticipantId ? <Text style={styles.message}>일정 참여자를 제거하는 중...</Text> : null}
          ...
          return (
            <View key={participant.participantId} style={styles.participantRow}>
              ...
              {participant.canRemove ? (
                <Pressable
                  accessibilityLabel={`${participant.displayName} 일정 참여자 제거`}
      ...
      <View style={styles.modalBackdrop}>
        <View style={styles.modalCard}>
          <Text style={styles.modalTitle}>
            {participant ? `${participant.displayName}님을 일정 참여자에서 제거할까요?` : '일정 참여자를 제거할까요?'}
          </Text>
          <Text style={styles.message}>
            제거되면 이 일정에 더 이상 접근할 수 없어요. 다시 초대하면 재참여할 수 있어요.
          </Text>
            ...
            >
              {isRemoving ? (
                <View style={styles.loadingRow}>
                  ...
                  <Text style={styles.dangerButtonText}>일정 참여자를 제거하는 중...</Text>
                </View>
              ) : (
