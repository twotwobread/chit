...
FROM expenses e
CROSS JOIN LATERAL (
  SELECT NULLIF(btrim($1::text), '') AS query
) search
LEFT JOIN expense_receipts er
  ON er.expense_id = e.id
 AND er.trip_id = e.trip_id
...
type ListTripExpensesByTripParams struct {
	SearchQuery string
	TripID      pgtype.UUID
}
...
func (q *Queries) ListTripExpensesByTrip(ctx context.Context, arg ListTripExpensesByTripParams) ([]ListTripExpensesByTripRow, error) {
	rows, err := q.db.Query(ctx, listTripExpensesByTrip, arg.SearchQuery, arg.TripID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
