...
func TestCreateAndListTripFlightsExposeOnlyMyPersonalDetail(t *testing.T) {
	backend := newFakeAuthBackend()
	owner := loginTestUserSession(t, backend, "owner-flight", "민수")
	member := loginTestUserSession(t, backend, "member-flight", "지영")
	tripID := createTestTrip(t, backend, owner.AccessToken)
	memberParticipantID := backend.addTripMember(t, tripID, member.UserID, "지영")
	...
	requestBody := []byte(fmt.Sprintf(`{
		...
		"passengerParticipantIds":[%q,%q]
	}`, ownerParticipantID, memberParticipantID))
	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodPost, "/trips/"+tripID+"/flights", bytes.NewReader(requestBody))
	...
	if recorder.Code != http.StatusCreated {
		t.Fatalf("expected create flight status %d, got %d with body %s", http.StatusCreated, recorder.Code, recorder.Body.String())
	}
	...
	if createBody.Flight.ID == "" || createBody.Flight.DisplayTitle != "KE 017" || createBody.Flight.FlightNumber == nil || *createBody.Flight.FlightNumber != "KE017" {
		t.Fatalf("unexpected created flight: %#v", createBody.Flight)
	}
	if createBody.Flight.Departure.AirportText != "ICN" || createBody.Flight.Departure.AirportCode == nil || *createBody.Flight.Departure.AirportCode != "ICN" || createBody.Flight.Departure.At != "2026-08-01T05:30:00Z" {
		t.Fatalf("unexpected departure mapping: %#v", createBody.Flight.Departure)
	}
	if createBody.Flight.MyPersonalDetail != nil {
		t.Fatalf("expected no auto-created personal detail, got %#v", createBody.Flight.MyPersonalDetail)
	}

	listRecorder := httptest.NewRecorder()
	listRequest := httptest.NewRequest(http.MethodGet, "/trips/"+tripID+"/flights", nil)
	listRequest.Header.Set("Authorization", "Bearer "+member.AccessToken)
	...
	if len(listBody.Flights) != 1 || len(listBody.Flights[0].Passengers) != 2 || listBody.Flights[0].MyPersonalDetail == nil {
...
func TestFlightBoardingPassUploadOpenAndDeleteRequiresCurrentPassenger(t *testing.T) {
	...
	owner := loginTestUserSession(t, backend, "owner-flight-attachment", "민수")
	member := loginTestUserSession(t, backend, "member-flight-attachment", "지영")
	tripID := createTestTrip(t, backend, owner.AccessToken)
	memberParticipantID := backend.addTripMember(t, tripID, member.UserID, "지영")
	...
	createRequest.Header.Set("Authorization", "Bearer "+owner.AccessToken)
	createRequest.Header.Set("Content-Type", "application/json")
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true, BoardingPassObjectStore: objectStore}).ServeHTTP(createRecorder, createRequest)
	if createRecorder.Code != http.StatusCreated {
		t.Fatalf("expected create flight status %d, got %d with body %s", http.StatusCreated, createRecorder.Code, createRecorder.Body.String())
	}
	var created struct {
		Flight struct {
			ID string `json:"id"`
	...
	if err := json.NewDecoder(createRecorder.Body).Decode(&created); err != nil {
		t.Fatalf("decode create flight: %v", err)
	}

	pngBytes := []byte{0x89, 'P', 'N', 'G', '\r', '\n', 0x1a, '\n', 0, 0, 0, 0, 'I', 'H', 'D', 'R'}
	nonPassengerUpload := httptest.NewRecorder()
	nonPassengerRequest := httptest.NewRequest(http.MethodPut, "/trips/"+tripID+"/flights/"+created.Flight.ID+"/my-detail/boarding-pass", bytes.NewReader(pngBytes))
	nonPassengerRequest.Header.Set("Authorization", "Bearer "+owner.AccessToken)
	nonPassengerRequest.Header.Set("Content-Type", "image/png")
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true, BoardingPassObjectStore: objectStore}).ServeHTTP(nonPassengerUpload, nonPassengerRequest)
	if nonPassengerUpload.Code != http.StatusForbidden {
		t.Fatalf("expected non-passenger upload status %d, got %d with body %s", http.StatusForbidden, nonPassengerUpload.Code, nonPassengerUpload.Body.String())
	}

	uploadRecorder := httptest.NewRecorder()
	uploadRequest := httptest.NewRequest(http.MethodPut, "/trips/"+tripID+"/flights/"+created.Flight.ID+"/my-detail/boarding-pass", bytes.NewReader(pngBytes))
	uploadRequest.Header.Set("Authorization", "Bearer "+member.AccessToken)
	uploadRequest.Header.Set("Content-Type", "image/png")
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true, BoardingPassObjectStore: objectStore}).ServeHTTP(uploadRecorder, uploadRequest)
	if uploadRecorder.Code != http.StatusOK {
		t.Fatalf("expected upload status %d, got %d with body %s", http.StatusOK, uploadRecorder.Code, uploadRecorder.Body.String())
	}
	if strings.Contains(uploadRecorder.Body.String(), "objectKey") || strings.Contains(uploadRecorder.Body.String(), "signed") {
		t.Fatalf("upload response leaked private object data: %s", uploadRecorder.Body.String())
	}

	openRecorder := httptest.NewRecorder()
	openRequest := httptest.NewRequest(http.MethodPost, "/trips/"+tripID+"/flights/"+created.Flight.ID+"/my-detail/boarding-pass/open-url", nil)
	openRequest.Header.Set("Authorization", "Bearer "+member.AccessToken)
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true, BoardingPassObjectStore: objectStore}).ServeHTTP(openRecorder, openRequest)
	if openRecorder.Code != http.StatusOK || openRecorder.Header().Get("Cache-Control") != "no-store" {
		t.Fatalf("expected open status 200/no-store, got %d cache=%q body=%s", openRecorder.Code, openRecorder.Header().Get("Cache-Control"), openRecorder.Body.String())
	}
	var opened struct {
		URL         string `json:"url"`
	...
	if opened.URL == "" || opened.ContentType != "image/png" || opened.ByteSize != len(pngBytes) {
		t.Fatalf("unexpected open response: %#v", opened)
	}

	deleteRecorder := httptest.NewRecorder()
	deleteRequest := httptest.NewRequest(http.MethodDelete, "/trips/"+tripID+"/flights/"+created.Flight.ID+"/my-detail/boarding-pass", nil)
	deleteRequest.Header.Set("Authorization", "Bearer "+member.AccessToken)
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true, BoardingPassObjectStore: objectStore}).ServeHTTP(deleteRecorder, deleteRequest)
...
func TestCreateMeetingAndOneOffEventVisibility(t *testing.T) {
	backend := newFakeAuthBackend()
	session := loginTestUserSession(t, backend, "meeting-owner", "민수")

	createMeetingRecorder := httptest.NewRecorder()
	createMeetingRequest := httptest.NewRequest(http.MethodPost, "/meetings", bytes.NewReader([]byte(`{"name":"  동네 친구들  "}`)))
	createMeetingRequest.Header.Set("Content-Type", "application/json")
	createMeetingRequest.Header.Set("Authorization", "Bearer "+session.AccessToken)
	...
	if meetingBody.Meeting.ID == "" || meetingBody.Meeting.Name != "동네 친구들" || meetingBody.Meeting.Visibility != "saved" {
		t.Fatalf("unexpected meeting response: %#v", meetingBody.Meeting)
	}
	...
	if createEventRecorder.Code != http.StatusCreated {
		t.Fatalf("expected create one-off event status %d, got %d with body %s", http.StatusCreated, createEventRecorder.Code, createEventRecorder.Body.String())
	}
	...
	if eventBody.Event.ID == "" || eventBody.Event.MeetingVisibility != "one_off" || eventBody.Event.Title != "성수 저녁" {
		t.Fatalf("unexpected one-off event response: %#v", eventBody.Event)
	}

	listRecorder := httptest.NewRecorder()
	listRequest := httptest.NewRequest(http.MethodGet, "/meetings", nil)
	listRequest.Header.Set("Authorization", "Bearer "+session.AccessToken)
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true}).ServeHTTP(listRecorder, listRequest)
	if listRecorder.Code != http.StatusOK {
		t.Fatalf("expected list meetings status %d, got %d with body %s", http.StatusOK, listRecorder.Code, listRecorder.Body.String())
	}
	var listBody struct {
		Meetings []struct {
			ID         string `json:"id"`
	...
	if len(listBody.Meetings) != 1 || listBody.Meetings[0].ID != meetingBody.Meeting.ID || listBody.Meetings[0].Visibility != "saved" {
		t.Fatalf("expected only saved meeting in list, got %#v", listBody.Meetings)
	}
}
...
func TestCreateEventWithExistingMeetingRequiresMembershipAndGetEventRequiresParticipant(t *testing.T) {
	...
	outsider := loginTestUserSession(t, backend, "event-outsider", "지영")

	createMeetingRecorder := httptest.NewRecorder()
	createMeetingRequest := httptest.NewRequest(http.MethodPost, "/meetings", bytes.NewReader([]byte(`{"name":"일본 여행 멤버"}`)))
	createMeetingRequest.Header.Set("Content-Type", "application/json")
	...
	outsiderRecorder := httptest.NewRecorder()
	outsiderRequest := httptest.NewRequest(http.MethodPost, "/events", bytes.NewReader([]byte(fmt.Sprintf(`{
		"title":"오사카 여행",
		"startDate":"2026-09-01",
		"endDate":"2026-09-03",
		"eventType":"trip",
	...
	}`, meetingBody.Meeting.ID))))
	outsiderRequest.Header.Set("Content-Type", "application/json")
	outsiderRequest.Header.Set("Authorization", "Bearer "+outsider.AccessToken)
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true}).ServeHTTP(outsiderRecorder, outsiderRequest)
	if outsiderRecorder.Code != http.StatusForbidden {
		t.Fatalf("expected outsider create event status %d, got %d with body %s", http.StatusForbidden, outsiderRecorder.Code, outsiderRecorder.Body.String())
	}
	...
	outsiderGetRecorder := httptest.NewRecorder()
	outsiderGetRequest := httptest.NewRequest(http.MethodGet, "/events/"+eventBody.Event.ID, nil)
	outsiderGetRequest.Header.Set("Authorization", "Bearer "+outsider.AccessToken)
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true}).ServeHTTP(outsiderGetRecorder, outsiderGetRequest)
	if outsiderGetRecorder.Code != http.StatusNotFound {
		t.Fatalf("expected outsider get event status %d, got %d with body %s", http.StatusNotFound, outsiderGetRecorder.Code, outsiderGetRecorder.Body.String())
	}
	...
	if ownerGetRecorder.Code != http.StatusOK {
		t.Fatalf("expected owner get event status %d, got %d with body %s", http.StatusOK, ownerGetRecorder.Code, ownerGetRecorder.Body.String())
	}
...
func TestPromoteTripMeetingHandlerPromotesOneOffMeeting(t *testing.T) {
	backend := newFakeAuthBackend()
	owner := loginTestUserSession(t, backend, "owner-promote-one-off", "민수")
	tripID := createTestTrip(t, backend, owner.AccessToken)
	beforeMeetingID := backend.trips[tripID].EventContext.MeetingID

	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodPost, "/trips/"+tripID+"/meeting/promotion", bytes.NewReader([]byte(`{"meetingName":"  성수 저녁 모임  "}`)))
	request.Header.Set("Authorization", "Bearer "+owner.AccessToken)
	request.Header.Set("Content-Type", "application/json")
	...
	if recorder.Code != http.StatusOK {
		t.Fatalf("expected promote status %d, got %d with body %s", http.StatusOK, recorder.Code, recorder.Body.String())
	}
	var body struct {
		Trip struct {
			ID           string `json:"id"`
	...
	if body.Trip.ID != tripID || body.Trip.EventContext.MeetingID != beforeMeetingID || body.Trip.EventContext.MeetingVisibility != "saved" || body.Trip.EventContext.MeetingName != "성수 저녁 모임" {
		t.Fatalf("unexpected promoted trip response: %#v", body.Trip)
	}
	if body.Meeting.ID != beforeMeetingID || body.Meeting.Name != "성수 저녁 모임" || body.Meeting.Visibility != "saved" {
		t.Fatalf("unexpected promoted meeting response: %#v", body.Meeting)
	}

	listRecorder := httptest.NewRecorder()
	listRequest := httptest.NewRequest(http.MethodGet, "/meetings", nil)
	listRequest.Header.Set("Authorization", "Bearer "+owner.AccessToken)
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true}).ServeHTTP(listRecorder, listRequest)
	if listRecorder.Code != http.StatusOK {
		t.Fatalf("expected list meetings status %d, got %d with body %s", http.StatusOK, listRecorder.Code, listRecorder.Body.String())
	}
	if !strings.Contains(listRecorder.Body.String(), "성수 저녁 모임") {
		t.Fatalf("expected promoted meeting in saved meeting list, got %s", listRecorder.Body.String())
	}
}

func TestPromoteTripMeetingHandlerErrors(t *testing.T) {
	backend := newFakeAuthBackend()
	owner := loginTestUserSession(t, backend, "owner-promote-errors", "민수")
	member := loginTestUserSession(t, backend, "member-promote-errors", "지은")
	oneOffTripID := createTestTrip(t, backend, owner.AccessToken)
	meetingResult, err := backend.CreateMeetingWithOwner(context.Background(), meetingdomain.CreateMeetingRecord{Name: "정식 모임", Visibility: meetingdomain.MeetingVisibilitySaved, CreatedBy: owner.UserID, OwnerDisplayName: "민수"})
	if err != nil {
		t.Fatalf("CreateMeetingWithOwner: %v", err)
	}
	savedTripID := createTestTripInMeeting(t, backend, owner.AccessToken, meetingResult.Meeting.ID)
	...
	tests := []struct {
		...
		{name: "invalid trip", tripID: "bad", token: owner.AccessToken, body: `{"meetingName":"성수 저녁 모임"}`, wantStatus: http.StatusBadRequest},
		{name: "blank name", tripID: oneOffTripID, token: owner.AccessToken, body: `{"meetingName":"   "}`, wantStatus: http.StatusBadRequest},
		{name: "forbidden", tripID: oneOffTripID, token: member.AccessToken, body: `{"meetingName":"성수 저녁 모임"}`, wantStatus: http.StatusForbidden},
		{name: "saved trip conflict", tripID: savedTripID, token: owner.AccessToken, body: `{"meetingName":"성수 저녁 모임"}`, wantStatus: http.StatusConflict},
		{name: "missing trip", tripID: "00000000-0000-0000-0000-000000000404", token: owner.AccessToken, body: `{"meetingName":"성수 저녁 모임"}`, wantStatus: http.StatusNotFound},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			recorder := httptest.NewRecorder()
			request := httptest.NewRequest(http.MethodPost, "/trips/"+tt.tripID+"/meeting/promotion", strings.NewReader(tt.body))
			request.Header.Set("Authorization", "Bearer "+tt.token)
			request.Header.Set("Content-Type", "application/json")
			NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true}).ServeHTTP(recorder, request)
...
func TestCreateManualTripPlaceHandlerDoesNotMutateDay(t *testing.T) {
	backend := newFakeAuthBackend()
	accessToken := loginTestUser(t, backend)
	tripID := createTestTrip(t, backend, accessToken)

	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodPost, "/trips/"+tripID+"/places", bytes.NewReader([]byte(`{"name":" 주식회사비플랜트 ","address":" 서울특별시 마포구 월드컵로 14길 108, 지층 ","placeType":"food"}`)))
	request.Header.Set("Content-Type", "application/json")
	request.Header.Set("Authorization", "Bearer "+accessToken)
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true}).ServeHTTP(recorder, request)

	if recorder.Code != http.StatusCreated {
		t.Fatalf("expected create status %d, got %d with body %s", http.StatusCreated, recorder.Code, recorder.Body.String())
	}
	var body struct {
		Place struct {
			ID        string `json:"id"`
	...
	if body.Place.ID == "" || body.Place.Name != "주식회사비플랜트" || body.Place.PlaceType != "food" {
		t.Fatalf("unexpected manual trip place response: %#v", body.Place)
	}
	if len(backend.dayLodgingPlaces) != 0 {
		t.Fatalf("manual trip place must not set day lodging: %#v", backend.dayLodgingPlaces)
	}
	if len(backend.dayScheduleItems) != 0 {
		t.Fatalf("manual trip place must not create schedule items: %#v", backend.dayScheduleItems)
	}
}
...
func TestGetUpdateDeleteTripExpenseHandlers(t *testing.T) {
	backend := newFakeAuthBackend()
	ownerToken := loginTestUser(t, backend)
	tripID := createTestTrip(t, backend, ownerToken)
	...
	}`, ownerParticipantID, ownerParticipantID, member.ID))
	createRecorder := httptest.NewRecorder()
	createRequest := httptest.NewRequest(http.MethodPost, "/trips/"+tripID+"/expenses", bytes.NewReader(createBody))
	createRequest.Header.Set("Content-Type", "application/json")
	createRequest.Header.Set("Authorization", "Bearer "+ownerToken)
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true}).ServeHTTP(createRecorder, createRequest)
	if createRecorder.Code != http.StatusCreated {
		t.Fatalf("expected create trip expense status %d, got %d with body %s", http.StatusCreated, createRecorder.Code, createRecorder.Body.String())
	}
	var created struct {
		Expense struct {
			ID                  string `json:"id"`
	...
	if created.Expense.IncludeInSettlement == nil || *created.Expense.IncludeInSettlement {
		t.Fatalf("expected created trip expense to be excluded from settlement, got %#v", created.Expense)
	}

	getRecorder := httptest.NewRecorder()
	getRequest := httptest.NewRequest(http.MethodGet, "/trips/"+tripID+"/expenses/"+created.Expense.ID, nil)
	getRequest.Header.Set("Authorization", "Bearer "+ownerToken)
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true}).ServeHTTP(getRecorder, getRequest)
	if getRecorder.Code != http.StatusOK {
		t.Fatalf("expected get trip expense status %d, got %d with body %s", http.StatusOK, getRecorder.Code, getRecorder.Body.String())
	}
	var got struct {
		Expense struct {
			ID                  string  `json:"id"`
	...
	patchBody := []byte(fmt.Sprintf(`{
		...
		"scheduleItemId":null,
		"includeInSettlement":true
	}`, member.ID, ownerParticipantID, member.ID))
	patchRecorder := httptest.NewRecorder()
	patchRequest := httptest.NewRequest(http.MethodPatch, "/trips/"+tripID+"/expenses/"+created.Expense.ID, bytes.NewReader(patchBody))
	...
	if patchRecorder.Code != http.StatusOK {
		t.Fatalf("expected patch trip expense status %d, got %d with body %s", http.StatusOK, patchRecorder.Code, patchRecorder.Body.String())
	}
	var patched struct {
		Expense struct {
			AnchorType      string  `json:"anchorType"`
	...
	deleteRecorder := httptest.NewRecorder()
	deleteRequest := httptest.NewRequest(http.MethodDelete, "/trips/"+tripID+"/expenses/"+created.Expense.ID, nil)
	deleteRequest.Header.Set("Authorization", "Bearer "+ownerToken)
	NewRouterWithConfig(backend, Config{AuthTokenSecret: "test-secret", AllowDevOAuth: true}).ServeHTTP(deleteRecorder, deleteRequest)
	if deleteRecorder.Code != http.StatusNoContent {
		t.Fatalf("expected delete trip expense status %d, got %d with body %s", http.StatusNoContent, deleteRecorder.Code, deleteRecorder.Body.String())
	}

	missingRecorder := httptest.NewRecorder()
	missingRequest := httptest.NewRequest(http.MethodGet, "/trips/"+tripID+"/expenses/"+created.Expense.ID, nil)
	...
	if missingRecorder.Code != http.StatusNotFound {
		t.Fatalf("expected missing trip expense status %d, got %d with body %s", http.StatusNotFound, missingRecorder.Code, missingRecorder.Body.String())
	}
}
...
func (b *fakeAuthBackend) addTripMember(t *testing.T, tripID string, userID string, displayName string) string {
	t.Helper()
	participant := tripdomain.Participant{
		ID:          testUUID(9000 + len(b.participants[tripID])),
		TripID:      tripID,
		UserID:      userID,
		Role:        tripdomain.RoleMember,
		DisplayName: displayName,
		JoinedAt:    time.Date(2026, 6, 22, 10, 0, 0, 0, time.UTC),
	}
	b.participants[tripID] = append(b.participants[tripID], participant)
...
func (b *fakeAuthBackend) GetMeetingCreator(_ context.Context, userID string) (meetingdomain.Creator, bool, error) {
	user, ok := b.users[userID]
	if !ok || b.deletedUsers[userID] {
		return meetingdomain.Creator{}, false, nil
	}
	return meetingdomain.Creator{ID: user.ID, DisplayName: user.DisplayName}, true, nil
}

func (b *fakeAuthBackend) CreateMeetingWithOwner(_ context.Context, record meetingdomain.CreateMeetingRecord) (meetingdomain.CreateMeetingResult, error) {
	b.nextMeeting++
	b.nextMeetingMember++
	now := time.Date(2026, 8, 22, 9, 0, 0, 0, time.UTC)
	meetingID := testUUID(8000 + b.nextMeeting)
	memberID := testUUID(9000 + b.nextMeetingMember)
	created := meetingdomain.Meeting{ID: meetingID, Name: record.Name, Visibility: record.Visibility, CreatedBy: record.CreatedBy, CreatedAt: now, UpdatedAt: now}
	owner := meetingdomain.MeetingMember{ID: memberID, MeetingID: meetingID, UserID: record.CreatedBy, Role: meetingdomain.RoleOwner, DisplayName: record.OwnerDisplayName, JoinedAt: now}
	b.meetings[meetingID] = created
	b.meetingMembers[meetingID] = append(b.meetingMembers[meetingID], owner)
	return meetingdomain.CreateMeetingResult{Meeting: created, OwnerMember: owner}, nil
}

func (b *fakeAuthBackend) ListSavedMeetingsByMemberUser(_ context.Context, userID string) ([]meetingdomain.MeetingListItem, error) {
	items := []meetingdomain.MeetingListItem{}
	for meetingID, members := range b.meetingMembers {
		...
		for _, member := range members {
			if member.UserID == userID {
				items = append(items, meetingdomain.MeetingListItem{ID: meeting.ID, Name: meeting.Name, Visibility: meeting.Visibility, MemberCount: len(members), MyRole: member.Role, CreatedAt: meeting.CreatedAt, UpdatedAt: meeting.UpdatedAt})
				break
			}
		}
	}
...
func (b *fakeAuthBackend) GetSavedMeetingForMember(_ context.Context, meetingID string, userID string) (meetingdomain.Meeting, bool, error) {
...
func (b *fakeAuthBackend) CreateTripWithOwner(_ context.Context, record tripdomain.CreateRecord) (tripdomain.CreateResult, error) {
	...
	if _, ok := b.meetings[meetingID]; !ok {
		b.meetings[meetingID] = meetingdomain.Meeting{ID: meetingID, Name: meetingName, Visibility: meetingVisibility, CreatedBy: record.CreatedBy, CreatedAt: now, UpdatedAt: now}
	}
	ownerMemberExists := false
	for _, member := range b.meetingMembers[meetingID] {
		if member.UserID == record.CreatedBy {
	...
	if !ownerMemberExists {
		b.nextMeetingMember++
		b.meetingMembers[meetingID] = append(b.meetingMembers[meetingID], meetingdomain.MeetingMember{ID: testUUID(16000 + b.nextMeetingMember), MeetingID: meetingID, UserID: record.CreatedBy, Role: meetingdomain.RoleOwner, DisplayName: record.OwnerDisplayName, JoinedAt: now})
	}
	...
	meetingMemberByUserID := map[string]meetingdomain.MeetingMember{}
	for _, member := range b.meetingMembers[meetingID] {
		meetingMemberByUserID[member.UserID] = member
	}
	eventParticipants := []meetingdomain.EventParticipant{}
	for index, participant := range participants {
		var meetingMemberID *string
		if member, ok := meetingMemberByUserID[participant.UserID]; ok {
			memberID := member.ID
			meetingMemberID = &memberID
		}
		eventParticipants = append(eventParticipants, meetingdomain.EventParticipant{ID: testUUID(17000 + b.nextTrip*10 + index), EventID: createdTrip.EventContext.EventID, MeetingMemberID: meetingMemberID, UserID: participant.UserID, Role: participant.Role, DisplayName: participant.DisplayName, JoinedAt: participant.JoinedAt})
	}
	b.eventParticipants[createdTrip.EventContext.EventID] = eventParticipants
...
func (b *fakeAuthBackend) PromoteTripMeeting(_ context.Context, record tripdomain.PromoteMeetingRecord) (tripdomain.PromoteMeetingResult, error) {
	tripRecord, ok := b.trips[record.TripID]
	if !ok {
		return tripdomain.PromoteMeetingResult{}, tripdomain.ErrNotFound
	}
	if tripRecord.EventContext == nil || tripRecord.EventContext.MeetingVisibility != meetingdomain.MeetingVisibilityOneOff {
		return tripdomain.PromoteMeetingResult{}, tripdomain.ErrConflict
	}
	meetingRecord, ok := b.meetings[tripRecord.EventContext.MeetingID]
	if !ok || meetingRecord.Visibility != meetingdomain.MeetingVisibilityOneOff {
		return tripdomain.PromoteMeetingResult{}, tripdomain.ErrConflict
	}
	now := time.Date(2026, 8, 23, 10, 0, 0, 0, time.UTC)
	meetingRecord.Name = record.MeetingName
	meetingRecord.Visibility = meetingdomain.MeetingVisibilitySaved
	meetingRecord.UpdatedAt = now
	b.meetings[meetingRecord.ID] = meetingRecord

	tripRecord.EventContext.MeetingName = record.MeetingName
	tripRecord.EventContext.MeetingVisibility = meetingdomain.MeetingVisibilitySaved
	tripRecord.UpdatedAt = now
	b.trips[record.TripID] = tripRecord

	if eventRecord, ok := b.events[tripRecord.EventContext.EventID]; ok {
		eventRecord.MeetingName = record.MeetingName
		eventRecord.MeetingVisibility = meetingdomain.MeetingVisibilitySaved
		eventRecord.UpdatedAt = now
		b.events[eventRecord.ID] = eventRecord
	}

	return tripdomain.PromoteMeetingResult{
		Trip: tripRecord,
		Meeting: tripdomain.PromotedMeeting{
			ID:         meetingRecord.ID,
			Name:       meetingRecord.Name,
			Visibility: meetingRecord.Visibility,
			CreatedBy:  meetingRecord.CreatedBy,
...
func (b *fakeAuthBackend) AcceptTripInvite(_ context.Context, record tripdomain.AcceptTripInviteRecord) (tripdomain.AcceptTripInviteResult, error) {
	...
	b.participants[foundInvite.TripID] = append(b.participants[foundInvite.TripID], participant)
	if foundTrip.EventContext != nil {
		meetingID := foundTrip.EventContext.MeetingID
		var meetingMemberID string
		for _, member := range b.meetingMembers[meetingID] {
			if member.UserID == record.UserID {
		...
		if meetingMemberID == "" {
			b.nextMeetingMember++
			meetingMemberID = testUUID(16000 + b.nextMeetingMember)
			b.meetingMembers[meetingID] = append(b.meetingMembers[meetingID], meetingdomain.MeetingMember{ID: meetingMemberID, MeetingID: meetingID, UserID: record.UserID, Role: meetingdomain.RoleMember, DisplayName: displayName, JoinedAt: record.Now})
		}
		b.nextEventMember++
		memberID := meetingMemberID
		b.eventParticipants[foundTrip.EventContext.EventID] = append(b.eventParticipants[foundTrip.EventContext.EventID], meetingdomain.EventParticipant{ID: testUUID(18000 + b.nextEventMember), EventID: foundTrip.EventContext.EventID, MeetingMemberID: &memberID, UserID: record.UserID, Role: meetingdomain.RoleMember, DisplayName: displayName, JoinedAt: record.Now})
	}
...
func (b *fakeAuthBackend) CreateManualTripPlace(_ context.Context, record tripdomain.CreateManualTripPlaceRecord) (tripdomain.TripPlaceSummary, error) {
	b.nextPlace++
	place := tripdomain.TripPlaceSummary{
		ID:        testUUID(6000 + b.nextPlace),
		Name:      record.Name,
...
func (b *fakeAuthBackend) GetTripSettlementInput(_ context.Context, tripID string) (tripdomain.SettlementInput, error) {
	...
	for _, tripExpense := range b.tripExpenses[tripID] {
		if !tripExpense.IncludeInSettlement {
			continue
		}
	...
	for key, dayExpenses := range b.dayExpenses {
		...
		for _, dayExpense := range dayExpenses {
			if !dayExpense.IncludeInSettlement {
				continue
			}
...
func (b *fakeAuthBackend) GetTripExpenseByID(_ context.Context, tripID string, expenseID string) (tripdomain.Expense, bool, error) {
	expense, ok := b.tripExpenseDetail[tripID+":"+expenseID]
	if !ok {
		return tripdomain.Expense{}, false, nil
	}
...
func (b *fakeAuthBackend) UpdateTripExpense(_ context.Context, record tripdomain.UpdateExpenseRecord) (tripdomain.Expense, error) {
	...
	expenseIndex := -1
	for index, expense := range b.tripExpenses[record.TripID] {
	...
	existingDetail, ok := b.tripExpenseDetail[record.TripID+":"+record.ExpenseID]
	if !ok {
		return tripdomain.Expense{}, tripdomain.ErrNotFound
	}
	...
	includeInSettlement := existingDetail.IncludeInSettlement
	...
	updatedTripExpense := tripdomain.DayExpenseListItem{
		...
		TripDayID:           nil,
		ScheduleItemID:      nil,
		ExpenseDate:         existingDetail.ExpenseDate,
	...
	b.tripExpenses[record.TripID][expenseIndex] = updatedTripExpense

	updatedDetail := tripdomain.Expense{
		ID:                  record.ExpenseID,
		TripID:              record.TripID,
		AnchorType:          "trip",
		TripDayID:           nil,
	...
	b.tripExpenseDetail[record.TripID+":"+record.ExpenseID] = updatedDetail
	return updatedDetail, nil
}

func (b *fakeAuthBackend) DeleteTripExpenseByID(_ context.Context, tripID string, expenseID string) (bool, error) {
	expenseIndex := -1
	for index, expense := range b.tripExpenses[tripID] {
	...
	b.tripExpenses[tripID] = append(b.tripExpenses[tripID][:expenseIndex], b.tripExpenses[tripID][expenseIndex+1:]...)
	delete(b.tripExpenseDetail, tripID+":"+expenseID)
	return true, nil
}
...
func (b *fakeAuthBackend) UpdateExpense(_ context.Context, record tripdomain.UpdateExpenseRecord) (tripdomain.Expense, error) {
	...
	includeInSettlement := existing.IncludeInSettlement
	if record.IncludeInSettlement != nil {
		includeInSettlement = *record.IncludeInSettlement
	}
...
func (b *fakeAuthBackend) CreateTripExpense(_ context.Context, record tripdomain.CreateTripExpenseRecord) (tripdomain.CreateTripExpenseResult, error) {
	...
	createdDetail := tripdomain.Expense{
		ID:                  expenseID,
		TripID:              record.TripID,
		AnchorType:          anchorType,
		TripDayID:           tripDayID,
	...
	if anchorType == "trip" {
		...
		b.tripExpenseDetail[record.TripID+":"+expenseID] = createdDetail
	} else if tripDayID != nil {
		b.dayExpenses[record.TripID+":"+*tripDayID] = append(b.dayExpenses[record.TripID+":"+*tripDayID], dayExpense)
	}

	return tripdomain.CreateTripExpenseResult{Expense: createdDetail}, nil
}
...
func (b *fakeAuthBackend) GetTripParticipantID(_ context.Context, tripID string, userID string) (string, error) {
	for _, participant := range b.participants[tripID] {
		if participant.UserID == userID {
			return participant.ID, nil
		}
	}
	return "", flightdomain.ErrNotFound
}

func (b *fakeAuthBackend) CreateFlightWithPassengers(_ context.Context, record flightdomain.CreateFlightRecord) (flightdomain.FlightDetail, error) {
	b.nextFlight++
	now := time.Date(2026, 6, 22, 12, 0, 0, 0, time.UTC)
	flightRecord := flightdomain.Flight{
		ID:              testUUID(10000 + b.nextFlight),
		TripID:          record.TripID,
	...
	passengers := b.flightPassengers(record.TripID, record.PassengerIDs)
	detail := flightdomain.FlightDetail{Flight: flightRecord, Passengers: passengers}
	b.flights[flightRecord.ID] = detail
	return detail, nil
}
...
func (b *fakeAuthBackend) ListFlights(_ context.Context, tripID string, userID string) ([]flightdomain.FlightDetail, error) {
	...
	for _, detail := range b.flights {
		if detail.TripID != tripID {
			continue
		}
		copyDetail := detail
		if personal, ok := b.personalDetails[detail.ID+":"+participantID]; ok {
			copyDetail.MyPersonalDetail = &personal
		} else if passengerListContainsFake(detail.Passengers, participantID) {
			personal := flightdomain.PersonalDetail{FlightID: detail.ID, TripID: tripID, PassengerParticipantID: participantID}
			copyDetail.MyPersonalDetail = &personal
		} else {
			copyDetail.MyPersonalDetail = nil
		}
		result = append(result, copyDetail)
	}
	sort.Slice(result, func(i, j int) bool {
		return result[i].Departure.At.Before(result[j].Departure.At)
	})
	return result, nil
}

func (b *fakeAuthBackend) GetFlight(_ context.Context, tripID string, flightID string, userID string) (flightdomain.FlightDetail, error) {
	detail, ok := b.flights[flightID]
	if !ok || detail.TripID != tripID {
		return flightdomain.FlightDetail{}, flightdomain.ErrNotFound
	}
	participantID, _ := b.GetTripParticipantID(context.Background(), tripID, userID)
	if personal, ok := b.personalDetails[flightID+":"+participantID]; ok {
		detail.MyPersonalDetail = &personal
	} else if passengerListContainsFake(detail.Passengers, participantID) {
		personal := flightdomain.PersonalDetail{FlightID: flightID, TripID: tripID, PassengerParticipantID: participantID}
		detail.MyPersonalDetail = &personal
	} else {
		detail.MyPersonalDetail = nil
	}
	return detail, nil
}

func (b *fakeAuthBackend) GetFlightPassengerParticipantID(_ context.Context, tripID string, flightID string, userID string) (string, error) {
	detail, ok := b.flights[flightID]
	if !ok || detail.TripID != tripID {
		return "", flightdomain.ErrNotFound
	}
	participantID, err := b.GetTripParticipantID(context.Background(), tripID, userID)
	if err != nil {
		return "", err
	}
...
func (b *fakeAuthBackend) UpsertPersonalDetail(_ context.Context, record flightdomain.UpsertPersonalDetailRecord) (flightdomain.PersonalDetail, error) {
	b.nextPersonal++
	now := time.Date(2026, 6, 22, 12, 0, 0, 0, time.UTC)
	key := record.FlightID + ":" + record.PassengerParticipantID
	detail := b.personalDetails[key]
	if detail.ID == "" {
		detail.ID = testUUID(11000 + b.nextPersonal)
		detail.FlightID = record.FlightID
		detail.TripID = record.TripID
		detail.PassengerParticipantID = record.PassengerParticipantID
		detail.CreatedByUserID = record.CreatedByUserID
		detail.CreatedAt = now
	}
	detail.ReservationNumber = record.ReservationNumber
	detail.Seat = record.Seat
	detail.CheckInURL = record.CheckInURL
	detail.UpdatedAt = now
	b.personalDetails[key] = detail
	return detail, nil
}

func (b *fakeAuthBackend) UpsertBoardingPassMetadata(_ context.Context, record flightdomain.UpsertBoardingPassMetadataRecord) (flightdomain.PersonalDetail, *flightdomain.BoardingPassObject, error) {
	b.nextPersonal++
	now := time.Date(2026, 6, 22, 12, 0, 0, 0, time.UTC)
	key := record.FlightID + ":" + record.PassengerParticipantID
	detail := b.personalDetails[key]
	var oldObject *flightdomain.BoardingPassObject
	if detail.BoardingPass.Exists {
		oldObject = &flightdomain.BoardingPassObject{Bucket: "test-bucket", ObjectKey: "old-object", ContentType: *detail.BoardingPass.ContentType, ByteSize: *detail.BoardingPass.ByteSize, UploadedAt: *detail.BoardingPass.UploadedAt}
	}
	if detail.ID == "" {
		detail.ID = testUUID(11000 + b.nextPersonal)
		detail.FlightID = record.FlightID
	...
	contentType := record.Object.ContentType
	byteSize := record.Object.ByteSize
	uploadedAt := record.Object.UploadedAt
	detail.BoardingPass = flightdomain.BoardingPassSummary{Exists: true, ContentType: &contentType, ByteSize: &byteSize, UploadedAt: &uploadedAt}
	detail.UpdatedAt = now
	b.personalDetails[key] = detail
	return detail, oldObject, nil
}

func (b *fakeAuthBackend) ClearBoardingPassMetadata(_ context.Context, tripID string, flightID string, passengerParticipantID string) (*flightdomain.BoardingPassObject, error) {
	key := flightID + ":" + passengerParticipantID
	detail := b.personalDetails[key]
	if !detail.BoardingPass.Exists {
		return nil, nil
	}
	oldObject := flightdomain.BoardingPassObject{Bucket: "test-bucket", ObjectKey: "old-object", ContentType: *detail.BoardingPass.ContentType, ByteSize: *detail.BoardingPass.ByteSize, UploadedAt: *detail.BoardingPass.UploadedAt}
	detail.BoardingPass = flightdomain.BoardingPassSummary{}
	b.personalDetails[key] = detail
	return &oldObject, nil
}

func (b *fakeAuthBackend) GetBoardingPassObject(_ context.Context, tripID string, flightID string, passengerParticipantID string) (flightdomain.BoardingPassObject, error) {
	detail := b.personalDetails[flightID+":"+passengerParticipantID]
	if !detail.BoardingPass.Exists {
		return flightdomain.BoardingPassObject{}, flightdomain.ErrNotFound
	}
	return flightdomain.BoardingPassObject{Bucket: "test-bucket", ObjectKey: "object", ContentType: *detail.BoardingPass.ContentType, ByteSize: *detail.BoardingPass.ByteSize, UploadedAt: *detail.BoardingPass.UploadedAt}, nil
}

type fakeBoardingPassObjectStore struct{}

func (s *fakeBoardingPassObjectStore) UploadBoardingPass(_ context.Context, objectKey string, contentType string, data []byte) (flightdomain.BoardingPassObject, error) {
	generation := "1"
	return flightdomain.BoardingPassObject{Bucket: "test-bucket", ObjectKey: objectKey, Generation: &generation, ContentType: contentType, ByteSize: len(data), UploadedAt: time.Date(2026, 6, 22, 12, 30, 0, 0, time.UTC)}, nil
}

func (s *fakeBoardingPassObjectStore) DeleteObject(_ context.Context, object flightdomain.BoardingPassObject) error {
	return nil
}
...
func (b *fakeAuthBackend) flightPassengers(tripID string, passengerIDs []string) []flightdomain.Passenger {
	participantsByID := map[string]tripdomain.Participant{}
	for _, participant := range b.participants[tripID] {
		participantsByID[participant.ID] = participant
	}
	passengers := make([]flightdomain.Passenger, 0, len(passengerIDs))
	for _, participantID := range passengerIDs {
		participant := participantsByID[participantID]
		passengers = append(passengers, flightdomain.Passenger{ParticipantID: participantID, DisplayName: participant.DisplayName})
	}
	return passengers
}
