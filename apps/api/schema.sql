...
CREATE TABLE schedule_items (
  ...
  item_kind text NOT NULL DEFAULT 'place',
  non_place_category text,
  non_place_title text,
  non_place_memo text,
  non_place_link text,
  transport_mode text,
  transport_reference_number text,
  transport_booking_reference text,
  transport_origin_text text,
  transport_destination_text text,
  transport_terminal_text text,
  transport_gate_text text,
  deleted_at timestamptz,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  ...
  CONSTRAINT schedule_items_place_title_check CHECK ((item_kind <> 'place' OR (place_title IS NOT NULL AND char_length(btrim(place_title)) BETWEEN 1 AND 120))),
  CONSTRAINT schedule_items_place_memo_check CHECK ((place_memo IS NULL OR char_length(place_memo) <= 1000)),
  CONSTRAINT schedule_items_item_kind_check CHECK (item_kind IN ('place', 'non_place')),
  CONSTRAINT schedule_items_non_place_category_check CHECK (non_place_category IS NULL OR non_place_category IN ('transport', 'rest', 'memo', 'reminder')),
  CONSTRAINT schedule_items_transport_mode_check CHECK (transport_mode IS NULL OR transport_mode IN ('flight', 'train', 'bus', 'ferry', 'other')),
  CONSTRAINT schedule_items_backing_check CHECK (((item_kind = 'place' AND trip_place_id IS NOT NULL AND place_title IS NOT NULL AND non_place_category IS NULL AND non_place_title IS NULL AND non_place_memo IS NULL AND non_place_link IS NULL AND transport_mode IS NULL AND transport_reference_number IS NULL AND transport_booking_reference IS NULL AND transport_origin_text IS NULL AND transport_destination_text IS NULL AND transport_terminal_text IS NULL AND transport_gate_text IS NULL) OR (item_kind = 'non_place' AND trip_place_id IS NULL AND place_title IS NULL AND place_memo IS NULL AND non_place_category IS NOT NULL AND non_place_title IS NOT NULL))),
  CONSTRAINT schedule_items_transport_fields_check CHECK ((item_kind = 'place' OR (non_place_category = 'transport' AND transport_mode IS NOT NULL) OR (non_place_category IN ('rest', 'memo', 'reminder') AND transport_mode IS NULL AND transport_reference_number IS NULL AND transport_booking_reference IS NULL AND transport_origin_text IS NULL AND transport_destination_text IS NULL AND transport_terminal_text IS NULL AND transport_gate_text IS NULL))),
