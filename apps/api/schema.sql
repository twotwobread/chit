...
CREATE TABLE schedule_items (
  ...
  CONSTRAINT schedule_items_backing_check CHECK (((item_kind = 'place' AND trip_place_id IS NOT NULL AND non_place_category IS NULL AND non_place_title IS NULL AND non_place_memo IS NULL AND non_place_link IS NULL AND transport_mode IS NULL AND transport_reference_number IS NULL AND transport_booking_reference IS NULL AND transport_origin_text IS NULL AND transport_destination_text IS NULL AND transport_terminal_text IS NULL AND transport_gate_text IS NULL) OR (item_kind = 'non_place' AND trip_place_id IS NULL AND non_place_category IS NOT NULL AND non_place_title IS NOT NULL))),
