...
상세 지출 선택 화면도 하나의 큰 outer card에 option card를 중첩하지 않고, 직접 입력 화면과 같은 card surface 구조로 보여준다. 영수증은 필수가 아니며 상세 직접 입력 화면의 `선택 정보` 그룹에 영수증 row를 반복 노출하지 않는다. OCR draft 적용 후에는 작은 상태 카드로 표시한다.
